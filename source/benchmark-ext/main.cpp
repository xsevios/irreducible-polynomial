...
std::vector<PolynomState> get_result(std::vector<Benchmark>& results, int index)
{
    return std::get<2>(results[index]);
}
...
int main(int argc, char **argv)
{
    ...
    ofstream berlekampResultsFile("berlekamp_results.txt", ios::trunc);
    ofstream cantorzassenhausResultsFile("cantorzassenhaus_results.txt", ios::trunc);
    ofstream rabinResultsFile("rabin_results.txt", ios::trunc);
    ...
    if (poly_path == nullptr)
    {
        for (int dim = minDim; dim < maxDim; dim++)
        {
            if (isPrime(dim))
            {
                ...
                {
                    std::vector<PolynomExt> polynoms = BenchmarkRunner::generatePolynoms(dim, dim, deg, deg,
                                                                                      polyCountForEach);
    ...
    else
    {
        ...
        std::vector<PolynomExt> polynomsBuf;
        ...
        for (auto& p : polynoms)
        {
            ...
            if (polynomsBuf.size() == polyCountForEach)
            {
                ...
                auto berlekampResults       = get_result(results, 0);
                auto cantorzassenhausResults= get_result(results, 1);
                auto rabinResults           = get_result(results, 2);

                for (auto res : berlekampResults)
                    berlekampResultsFile << (res == PolynomState::IRREDUCIBLE) << std::endl;

                for (auto res : cantorzassenhausResults)
                    cantorzassenhausResultsFile << (res == PolynomState::IRREDUCIBLE) << std::endl;

                for (auto res : rabinResults)
                    rabinResultsFile << (res == PolynomState::IRREDUCIBLE) << std::endl;
    ...
    berlekampResultsFile.close();
    cantorzassenhausResultsFile.close();
    rabinResultsFile.close();
